...
![](https://tva1.sinaimg.cn/large/0081Kckwly1gkq7dlr6c6j32kc08q40o.jpg)

如果入口文件`main.js`没有返回值，即`_entry_return_`为`undefined`，同样打印下`myLibrary`，结果如下：
![](https://tva1.sinaimg.cn/large/0081Kckwly1gkq7fvc7xqj32ka040jsg.jpg)

## 7.libraryExport

根据上例，可以看到原始导出下的`_entry_return_`的数据结构不够友好。在使用的时候还需要利用对象结构寻找实际要的属性。那么`libraryTarget`可以用来解决这个问题的。
...
- `libraryExport: ['MyModule', 'MySubModule']`: `_entry_return_.MyModule.MySubModule`。
...
- `commonjs`: `commonJs`1.0版本的规范。`exports["myLibrary"] = _entry_return_`。
- `commonjs2`: `commonJs`2.0版本的规范。`module.exports = _entry_return_`。
